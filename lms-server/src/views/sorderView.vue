<template>
    <div :class="collapse==true?'SorderViewOne':'SorderView'">
        <!-- 有数据展示区域 -->
        <div v-if="tableData.length!=0">
            ...
            <el-form :inline="true" :model="form" class="demo-form-inline" ref="form" :hide-required-asterisk="true">
                ...
                <el-form-item label="收货员" prop="sman" v-if="form.type=='发货员'">
                    <el-input  type="text" v-model="form.sman" autocomplete="off"   placeholder="请输入收货员" size="samll"  clearable v-on:change="handleinput5($event)"></el-input>
                </el-form-item>
                <el-form-item label="收货日期" prop="stime" v-if="form.type=='发货时间'">
                    <el-date-picker v-model="form.stime" type="datetime" placeholder="选择日期时间" style="width:100%;" v-on:blur="handleinput6($event)"></el-date-picker>
                </el-form-item>
                <!-- <el-form-item>
            ...
            >
                ...
                width="120">
                    <template slot-scope="scope">
                        <span :class="activeFlag4==true?'active':''">{{ scope.row.sman }}</span>
                    </template>
                </el-table-column>
                <el-table-column
                label="发货日期"
                ...
                width="120">
                    <template slot-scope="scope">
                        <span :class="activeFlag5==true?'active':''">{{ scope.row.stime }}</span>
                    </template>
                </el-table-column>
                <el-table-column
                label="添加时间"
            ...
            >
                <el-form :model="packageForm" status-icon :rules="rules" ref="packageForm" label-width="120px" class="packageform" :hide-required-asterisk="true">
                    ...
                    <el-form-item label="发货员" prop="sman">
                        <el-input type="text" v-model="packageForm.sman" autocomplete="off"></el-input>
                    </el-form-item>
                    <el-form-item label="发货时间" prop="stime">
                        <el-date-picker v-model="packageForm.stime" type="datetime" placeholder="选择日期时间" style="width:100%;" v-on:blur="handleblur"></el-date-picker>
                    </el-form-item>
                    <el-form-item  label="备注" prop="sremark" class="remark">
                        <el-input type="textarea"  maxlength="100" show-word-limit  v-model="packageForm.sremark"  clearable  placeholder="请输入内容" ></el-input>
                    </el-form-item>
                    <el-form-item>
...
export default {
    ...
    watch: {
        ...
        //         this.activeFlag2=false
        //         this.tableData=this.tableData.filter((item,index)=>{
        //             console.log("订单的发货日期",item.stime)
        //             if(item.stime==time){
        //                 return item
        //             }
        //         })
    ...
    methods:{
            ...
            .then((res)=>{
                ...
                newList.forEach((item,index)=>{
                    ...
                    if(item.stime==""){
                        ...
                        item.stime=this.formateDate(item.stime)
                    }
                })
                // 将数组按照时间从大到小排序
        ...
        handleblur(){
            var date=new Date(this.packageForm.stime).getTime()
            var date1=new Date(this.packageForm.ctime).getTime()
            console.log("选择的时间",date,this.packageForm.ctime)
            if(date<=date1){
                this.$message({
                    message:"发货时间不能早于或等于收货时间",
                    type:"error"
                })
                this.packageForm.stime=''
            }
        },
        // 删除按钮

<template>
    <div :class="collapse==true ? 'CorderOne' : 'Corder'">
        <!-- 寄件人部分###################### -->
        <div  class="corderForm" v-if="this.codes.length!=0">
            <h3>添加运单收货</h3>
            <el-form :model="spackageForm" status-icon :rules="rules" ref="spackageForm" label-width="120px" class="spackageForm" :hide-required-asterisk="true">
                ...
                <el-form-item label="收货员" prop="cman">
                    <el-input type="text" v-model="spackageForm.cman" autocomplete="off"></el-input>
                </el-form-item>
                <el-form-item label="收货时间" prop="ctime">
                    <el-date-picker v-model="spackageForm.ctime" type="datetime" placeholder="选择日期时间" style="width:100%;" v-on:blur="handleblur">
                </el-date-picker>
                </el-form-item>
                <el-form-item  label="备注" prop="cremark" class="remark">
                    <el-input type="textarea"  maxlength="100" show-word-limit  v-model="spackageForm.cremark"  clearable  placeholder="请输入内容" ></el-input>
                </el-form-item>
                <el-form-item>
    ...
    methods:{
        ...
        enter(val){
            ...
            .then((res)=>{
                ...
                // 获取管理员的名字--赋值到表单中
                this.spackageForm.cman=this.$store.state.user.name
            })
            .catch((err)=>{
        ...
        handleblur(){
            var date=new Date(this.spackageForm.ctime).getTime()
            var date1=new Date(this.spackageForm.ordertime).getTime()
        ...
        spackageModify(spackageForm){
            this.$refs[spackageForm].validate((valid) => {
                if (valid) {
                    // this.spackageForm.ctime=this.blur(this.spackageForm.ctime)
                    // 改变订单的状态
                    this.spackageForm.progress="下单成功"

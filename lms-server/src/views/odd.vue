    ...
    <div :class="collapse===true ? 'OddOne' : 'Odd'">
        ...
        <div v-if="tableData.length!=0">
            ...
            <el-form :inline="true" :model="form" class="demo-form-inline" ref="form" :hide-required-asterisk="true">
                ...
                <el-form-item label="订单进度" prop="progress" v-if="form.type=='订单进度'">
                    <el-input  type="text" v-model="form.progress" autocomplete="off"   placeholder="请输入订单进度" size="samll"  clearable v-on:change="handleinput5($event)"></el-input>
                </el-form-item>
                <!-- <el-form-item>
            ...
            >
                ...
                width="120">
                    <template slot-scope="scope">
                        <span :class="activeFlag4==true?'active':''">{{ scope.row.progress }}</span>
                    </template>
                </el-table-column>
                <el-table-column
                label="添加时间"
                ...
                <el-table-column label="操作" align="center" width="180" fixed="right">
                    <template slot-scope="scope">
                        <el-button
                        size="mini"
                        @click="handleEdit(scope.$index, scope.row)" :disabled="scope.row.progress=='货物已从物流公司发出' || scope.row.progress=='运单已成功配送'?true:false">编辑</el-button>
                        <el-button
                        size="mini"
                        type="danger"
                        @click="handleDelete(scope.$index, scope.row)" :disabled="scope.row.progress=='货物已从物流公司发出' || scope.row.progress=='运单已成功配送'?true:false">删除</el-button>
                    </template>
                </el-table-column>
            </el-table>
            <!-- 分页的部分 -->
...
<script>
import {byName} from '../api/index.js'
import bus from '../assets/js/bus.js'
import pmd from '../components/paomadeng.vue'
import {Modify,Delete} from '../api/index.js'
export default {
    data(){
        /* 自定义的验证规则 */
    ...
    methods:{
        ...
        // 向后台发起请求---获取数据
        getData(){
            byName(this.$store.state.user.name)
            .then((res)=>{
                console.log("根据用户名查询成功",res)
                this.currentPage=this.currentIndex;
                res.info.forEach((item,index)=>{
                    let date = new Date(item.ordertime);
                    item.ordertime=date.getFullYear() + "-" + ((date.getMonth() + 1)<10?"0"+(date.getMonth()+1):(date.getMonth())) + "-" + (date.getDate() < 10 ? '0' + date.getDate() : date.getDate()) + " " + date.getHours() + ":" + date.getMinutes() + ":" + date.getSeconds();
                })
                // 将数组按照时间从大到小排序
                this.tableData=res.info.sort((a,b)=>{

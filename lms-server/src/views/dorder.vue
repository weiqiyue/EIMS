<template>
    <div :class="collapse==true ? 'DorderOne' : 'Dorder'">
        <!-- 寄件人部分###################### -->
        <div class="dorderForm" v-if="this.codes.length!=0">
            <h3>添加运单配送</h3>
            <el-form :model="spackageForm" status-icon :rules="rules" ref="spackageForm" label-width="120px" class="spackageForm" :hide-required-asterisk="true">
                ...
                <el-form-item label="配送时间" prop="dtime">
                    <el-date-picker v-model="spackageForm.dtime" type="datetime" placeholder="选择日期时间" style="width:100%;" v-on:blur="handleblur">
                </el-date-picker>
                </el-form-item>
                <el-form-item  label="备注" prop="dremark" class="remark">
                    <el-input type="textarea"  maxlength="100" show-word-limit  v-model="spackageForm.dremark"  clearable  placeholder="请输入内容" ></el-input>
                </el-form-item>
                <el-form-item>
...
export default {
    ...
    methods:{
        ...
        handleblur(){
            var date=new Date(this.spackageForm.dtime).getTime()
            var date1=new Date(this.spackageForm.stime).getTime()
        ...
        spackageModify(spackageForm){
            this.$refs[spackageForm].validate((valid) => {
                if (valid) {
                    ...
                    // 改变订单的状态
                    this.spackageForm.progress="货物已从物流公司发出"
                    // 更新订单信息

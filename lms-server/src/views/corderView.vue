<template>
    <div :class="collapse==true?'CorderViewOne':'CorderView'">
        <!-- 有数据展示区域 -->
        <div v-if="tableData.length!=0">
            ...
            <el-form :inline="true" :model="form" class="demo-form-inline" ref="form" :hide-required-asterisk="true">
                ...
                <el-form-item label="收货员" prop="cman" v-if="form.type=='收货员'">
                    <el-input  type="text" v-model="form.cman" autocomplete="off"   placeholder="请输入收货员" size="samll"  clearable v-on:change="handleinput5($event)"></el-input>
                </el-form-item>
                <el-form-item label="收货日期" prop="ctime" v-if="form.type=='收货时间'">
                    <el-date-picker v-model="form.ctime" type="datetime" placeholder="选择日期时间" style="width:100%;" v-on:blur="handleinput6($event)"></el-date-picker>
                </el-form-item>
                <!-- <el-form-item>
            ...
            >
                ...
                width="120">
                    <template slot-scope="scope">
                        <span :class="activeFlag4==true?'active':''">{{ scope.row.cman }}</span>
                    </template>
                </el-table-column>
                <el-table-column
                label="收货日期"
                ...
                width="120">
                    <template slot-scope="scope">
                        <span :class="activeFlag5==true?'active':''">{{ scope.row.ctime }}</span>
                    </template>
                </el-table-column>
                <el-table-column
                label="添加时间"
            ...
            >
                <el-form :model="packageForm" status-icon :rules="rules" ref="packageForm" label-width="120px" class="packageform" :hide-required-asterisk="true">
                    ...
                    <el-form-item label="发货员" prop="cman">
                        <el-input type="text" v-model="packageForm.cman" autocomplete="off"></el-input>
                    </el-form-item>
                    <el-form-item label="发货时间" prop="ctime">
                        <el-date-picker v-model="packageForm.ctime" type="datetime" placeholder="选择日期时间" style="width:100%;" v-on:blur="handleblur"></el-date-picker>
                    </el-form-item>
                    <el-form-item  label="备注" prop="cremark" class="remark">
                        <el-input type="textarea"  maxlength="100" show-word-limit  v-model="packageForm.cremark"  clearable  placeholder="请输入内容" ></el-input>
                    </el-form-item>
                    <el-form-item>
...
<script>
import {allPackage,Modify,Delete} from '../api/index.js'
...
export default {
    data(){
        ...
        return{
            ...
            rules:{
                ...
                cman:[
                    {required:true,message:"收货员不能为空",trigger:"blur"}
                ],
                packagenum:[
    ...
    watch: {
        ...
        //         this.activeFlag2=false
        //         this.tableData=this.tableData.filter((item,index)=>{
        //             console.log("订单的收货日期",item.ctime)
        //             if(item.ctime==time){
        //                 return item
        //             }
        //         })
    ...
    methods:{
            ...
            .then((res)=>{
                ...
                newList.forEach((item,index)=>{
                    ...
                    if(item.ctime==""){
                        ...
                        item.ctime=this.formateDate(item.ctime)
                    }
                })
                // 将数组按照时间从大到小排序
        ...
        handleblur(){
            var date=new Date(this.packageForm.ctime).getTime()
            var date1=new Date(this.packageForm.ordertime).getTime()
            console.log("选择的时间",date,this.packageForm.ordertime)
            if(date<=date1){
                this.$message({
                    message:"收货时间不能早于或等于下单时间",
                    type:"error"
                })
                this.packageForm.ctime=''
            }
        },
        // 删除按钮

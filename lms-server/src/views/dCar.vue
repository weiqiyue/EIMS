<template>
    <div :class="collapse==true?'DcarOne':'Dcar'">
        <!-- 有数据展示区域 -->
        <div v-if="tableData.length!=0">
            ...
            <el-form :inline="true" :model="form" class="demo-form-inline" ref="form" :hide-required-asterisk="true">
                ...
                <el-form-item label="联系电话" prop="carphone" v-if="form.type=='联系电话'">
                    <el-input  type="text" v-model="form.carphone" autocomplete="off"   placeholder="请输入驾驶员电话" size="samll"  clearable v-on:change="handleinput4($event)"></el-input>
                </el-form-item>
                <!-- <el-form-item>
            ...
            >
                ...
                width="120">
                    <template slot-scope="scope">
                        <span :class="activeFlag3==true?'active':''">{{ scope.row.carphone }}</span>
                    </template>
                </el-table-column>
                <el-table-column
                label="路线信息"
...
export default {
    ...
    watch: {
        ...
        'form.type'(val){
            ...
            if(this.form.carphone!=''){
                this.form.carphone=''
            }
        },
        // 监听  驾驶员姓名  筛选
        // 'form.carman'(val){
        //     console.log("监听的驾驶员",val)
        //     if(val!=""){
    ...
    methods:{
        ...
        del(){
            console.log("点击了确定删除")
            // 根据id(订单号)删除----在这里是变相的修改--将配车部分的信息置空 以及  订单状态---根据id更新
            this.Del.carman=""
            this.Del.carcode=""
            this.Del.carline=""
            this.Del.carphone=""
            this.Del.progress='货物已从物流公司发出'
            console.log("删除时的配车表单",this.Del)
            Modify(this.Id,this.Del)
            .then((res)=>{
                console.log("根据id更新配车成功",res)
                // 弹出提示
                this.$message({
                    message:"操作成功",
                    type:"success"
        ...
        carpackageModify(packageForm){
            this.$refs[packageForm].validate((valid) => {
                if (valid) {
                    // 根据订单号(_id)修改数据
                    console.log("修改时的配车表单",this.packageForm)
